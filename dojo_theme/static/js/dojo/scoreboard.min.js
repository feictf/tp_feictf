    ...
    .then((result) => {
      ...
      if (page != null) {
        ...
        for (let i = minPage; i <= maxPage; i++) {
          ...
          pageButton.addClass(
            i == page
              ? "scoreboard-page-selected"
              : "scoreboard-page-unselected"
          );
...
// on select category create table
const select = document.getElementById("top-category-select");
select.addEventListener("change", (event) => {
  loadTopCategory(event.target.value);
});

function loadTopCategory(category) {
  const topCategory = $(`#top-category`);
  $("#top-category-wrapper").css({ display: "block" });
  var endpoint = `/pwncollege_api/v1/scoreboard/topCategory/` + category;
      ...
      standings.forEach((team) => {
        const row = $(`
            <tr>
              ...
              <td class="p-0">
                <img src="${team.symbol}" class="scoreboard-symbol">
              </td>
              <td>
                <a href="${team.url}" class="top-category-name text-decoration-none">
                </a>
              </td>
              <td>
              </td>
            ...
            `);
        row.find(".top-category-name").text(team.name.slice(0, 50));
        if (result.me && team.place == result.me.place)
          row.addClass("scoreboard-row-me");
        topCategory.append(row);
      });

      if (page != null) {
        const topCategoryPages = $(`#top-category-pages`);
        topCategoryPages.empty();
        const minPage = Math.max(1, page - 5);
        const maxPage = Math.min(page + 5, result.num_pages);
        for (let i = minPage; i <= maxPage; i++) {
          const pageButton = $(`
          ...
          topCategoryPages.append(pageButton);
        }
      }
    });
}
